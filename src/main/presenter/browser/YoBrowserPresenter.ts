...
// Temporary stub implementations for unimplemented modules
class BrowserTab {
  constructor(
    public contents: WebContents,
    public cdpManager: CDPManager,
  ...
  async navigate(_url: string, _timeoutMs?: number): Promise<void> {}
  async takeScreenshot(_options?: ScreenshotOptions): Promise<string> {
    return ''
  }
  async extractDOM(_selector?: string): Promise<string> {
    return ''
  }
  async evaluateScript(_script: string): Promise<unknown> {
    return null
  }
  destroy(): void {}
}
...
class ScreenshotManager {
  constructor(_cdpManager: CDPManager) {}
}
...
class BrowserToolManager {
  constructor(_presenter: YoBrowserPresenter) {}
  getToolDefinitions(): BrowserTool[] {
    return []
  }
  async executeTool(
    _name: string,
...
const zodToJsonSchema = (_schema: unknown): unknown => ({})

const clearYoBrowserSessionData = async (): Promise<void> => {}
